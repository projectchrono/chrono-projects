...
void ShowUsage(std::string name) {
    std::cout << "usage: " + name +
                     " <json_file> <out_dir> <run_mode: 0-settling, 1-testing> <checkpoint_file abs path, "
...
int main(int argc, char* argv[]) {
    ...
    float wheel_radius = hmmwv.GetVehicle().GetWheel(front_axle, LEFT)->GetTire()->GetRadius() * L_mks_to_cgs;
    float wheel_mass = hmmwv.GetVehicle().GetWheel(front_axle, LEFT)->GetTire()->GetMass() * M_mks_to_cgs;
    ...
    switch (wheel_type) {
        case WHEEL_TYPE::GROUSER:
            ...
            wheel_mesh_filename = granular::GetDataFile("test_GRAN_HMMWV/grouser_wheel.obj");
            break;
        case WHEEL_TYPE::DETAILED:
            scaling.x = L_mks_to_cgs;
            scaling.y = L_mks_to_cgs;
            scaling.z = L_mks_to_cgs;
            wheel_mesh_filename = granular::GetDataFile("test_GRAN_HMMWV/hmmwv_tire_detailed.obj");
            break;
        case WHEEL_TYPE::LUGGED:
            scaling.x = L_mks_to_cgs;
            scaling.y = L_mks_to_cgs;
            scaling.z = L_mks_to_cgs;
            wheel_mesh_filename = granular::GetDataFile("test_GRAN_HMMWV/hmmwv_tire_lugged.obj");
            break;
    }
    ...
    gran_collision_bodies.push_back(std::pair<std::string, std::shared_ptr<ChBody>>(
        wheel_mesh_filename, hmmwv.GetVehicle().GetWheel(front_axle, LEFT)));
    gran_collision_bodies.push_back(std::pair<std::string, std::shared_ptr<ChBody>>(
        wheel_mesh_filename, hmmwv.GetVehicle().GetWheel(front_axle, RIGHT)));
    gran_collision_bodies.push_back(std::pair<std::string, std::shared_ptr<ChBody>>(
        wheel_mesh_filename, hmmwv.GetVehicle().GetWheel(rear_axle, LEFT)));
    gran_collision_bodies.push_back(std::pair<std::string, std::shared_ptr<ChBody>>(
    ...
    if (run_mode == RUN_MODE::TESTING) {
        ...
        double wheel_z =
            hmmwv.GetVehicle().GetWheel(front_axle, RIGHT)->GetPos().z() * L_mks_to_cgs - 1.1 * wheel_radius;
        double rear_wheel_x =
            hmmwv.GetVehicle().GetWheel(rear_axle, RIGHT)->GetPos().x() * L_mks_to_cgs - 1.1 * wheel_radius;
        ...
        while (curr_time < params.time_end) {
            ...
            if (sim_frame % render_steps == 0) {
                ...
                // Write chassis
                writeMeshFrames(outstream, *hmmwv.GetChassis()->GetBody(),
                                granular::GetDataFile("test_GRAN_HMMWV/hmmwv_chassis.obj"), L_mks_to_cgs, gran_offset);
