...
#ifndef CHFILTERFULLSCREENVISUALIZE_H
#define CHFILTERFULLSCREENVISUALIZE_H
...
class CH_SENSOR_API ChFilterFullScreenVisualize : public ChFilter {
  public:
    /// Class constructor
    /// @param w Width of the window to create
    /// @param h Height of the window to create
    /// @param name String name of the filter
    ChFilterFullScreenVisualize(int w, int h, std::string name = "ChFilterFullScreenVisualize", bool is_fullscreen = true);

    /// Class destructor
    virtual ~ChFilterFullScreenVisualize();
  ...
  protected:
    ...
    int m_h;                         ///< height of the window
		...
		bool m_is_fullscreen = false; ///< for creating a full screen window
};
