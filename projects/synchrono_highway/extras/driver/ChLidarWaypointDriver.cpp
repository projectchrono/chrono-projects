...
ChLidarWaypointDriver::ChLidarWaypointDriver(ChVehicle& vehicle,
    ...
    : ChDriver(vehicle), m_lidar(lidar), m_target_speed(target_speed), m_path(path), m_mode(NOMINAL) {
...
void ChLidarWaypointDriver::SetInputDataFile(const std::string& filename) {
    // Embed a DataDriver.
    m_data_driver = chrono_types::make_shared<ChDataDriver>(m_vehicle, filename, false);
    m_data_driver->Initialize();
}

void ChLidarWaypointDriver::SetInputMode(InputMode mode) {
    m_mode = mode;
    switch (mode) {
        case DATAFILE:
            if (m_data_driver) {
    ...
    if (m_mode == NOMINAL) {
        ...
        ChVector<double> curvature_location =
            ...
            curve_location_const *
                (m_acc_driver->GetSteeringController().GetTargetLocation() - m_vehicle.GetVehiclePos());
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
std::string ChLidarWaypointDriver::GetInputModeAsString() const {
    switch (m_mode) {
        case NOMINAL:
            return std::string("Input mode: NOMINAL");
        case DATAFILE:
            return std::string("Input mode: DATAFILE");
    }
    return std::string("");
}
