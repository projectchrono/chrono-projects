...
void RemoveCollisionModel(ChSystemParallelDVI* system, ChCollisionModel* model) {
	...
	for (int j = 0; j < pmodel->GetNObjects(); j++) {
		...
		if (removed) {
			//we removed a model, all of the starts are off by one for indices past the index removed, decrement all starts before removing a second model
...
void CreateBase(ChSystemParallelDVI* system) {
	...
	if (simulation_mode == TURN) {
		...
		InitializeObject(bottom_plate, 1, material, Vector(-30, -123, 0) + c_pos, Quaternion(1, 0, 0, 0), true, true, 2, 2);
