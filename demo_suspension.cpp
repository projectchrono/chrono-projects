 ...
 public:
  ...
  MySimpleCar(ChSystemParallelDVI* my_system) {
    ...
    link_revoluteRF->Initialize(
        wheelRF,
        spindleRF,
        ChCoordsys<>(ChVector<>(0.91, -0.026, 1.648965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_distRFU1->Initialize(
        truss, spindleRF, false, ChVector<>(0.446, 0.245, 1.640965), ChVector<>(0.716, 0.215, 1.635965));
    my_system->AddLink(link_distRFU1);

    link_distRFU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, front, upper, 2
    link_distRFU2->Initialize(
        truss, spindleRF, false, ChVector<>(0.478, 0.196, 1.420965), ChVector<>(0.716, 0.215, 1.635965));
    my_system->AddLink(link_distRFU2);

    link_distRFL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, front, lower, 1
    link_distRFL1->Initialize(
        truss, spindleRF, false, ChVector<>(0.307, 0, 1.911965), ChVector<>(0.787, -0.118, 1.652965));
    my_system->AddLink(link_distRFL1);

    link_distRFL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, front, lower, 2
    link_distRFL2->Initialize(
        truss, spindleRF, false, ChVector<>(0.307, 0, 1.465965), ChVector<>(0.787, -0.118, 1.652965));
    ...
    link_springRF->Initialize(
        truss, spindleRF, false, ChVector<>(0.498, 0.323, 1.792965), ChVector<>(0.543, -0.047, 1.785965));
    ...
    link_distRSTEER->Initialize(
        truss, spindleRF, false, ChVector<>(0.448, 0.054, 1.438965), ChVector<>(0.821, -0.016, 1.512965));
    ...
    link_distLFU1->Initialize(
        truss, spindleLF, false, ChVector<>(-0.446, 0.245, 1.640965), ChVector<>(-0.716, 0.215, 1.635965));
    my_system->AddLink(link_distLFU1);

    link_distLFU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, front, upper, 2
    link_distLFU2->Initialize(
        truss, spindleLF, false, ChVector<>(-0.478, 0.196, 1.420965), ChVector<>(-0.716, 0.215, 1.635965));
    my_system->AddLink(link_distLFU2);

    link_distLFL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, front, lower, 1
    link_distLFL1->Initialize(
        truss, spindleLF, false, ChVector<>(-0.307, 0, 1.911965), ChVector<>(-0.787, -0.118, 1.652965));
    my_system->AddLink(link_distLFL1);

    link_distLFL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, front, lower, 2
    link_distLFL2->Initialize(
        truss, spindleLF, false, ChVector<>(-0.307, 0, 1.465965), ChVector<>(-0.787, -0.118, 1.652965));
    my_system->AddLink(link_distLFL2);

    // .. create the spring between the truss and the spindle
    link_springLF = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springLF->Initialize(
        truss, spindleLF, false, ChVector<>(-0.498, 0.323, 1.792965), ChVector<>(-0.543, -0.047, 1.785965));
    ...
    link_distLSTEER->Initialize(
        truss, spindleLF, false, ChVector<>(-0.448, 0.054, 1.438965), ChVector<>(-0.821, -0.016, 1.512965));
    ...
    link_revoluteRB->Initialize(
        wheelRB,
        spindleRB,
        ChCoordsys<>(ChVector<>(0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_engineR->Initialize(
        wheelRB, truss, ChCoordsys<>(ChVector<>(0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_distRBU1->Initialize(
        truss, spindleRB, false, ChVector<>(0.462, 0.228, -1.339965), ChVector<>(0.716, 0.216, -1.652965));
    my_system->AddLink(link_distRBU1);

    link_distRBU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, back, upper, 2
    link_distRBU2->Initialize(
        truss, spindleRB, false, ChVector<>(0.462, 0.224, -1.611965), ChVector<>(0.716, 0.216, -1.652965));
    my_system->AddLink(link_distRBU2);

    link_distRBL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, back, lower, 1
    link_distRBL1->Initialize(
        truss, spindleRB, false, ChVector<>(0.307, 0, -1.465965), ChVector<>(0.787, -0.118, -1.652965));
    my_system->AddLink(link_distRBL1);

    link_distRBL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // right, back, lower, 2
    link_distRBL2->Initialize(
        truss, spindleRB, false, ChVector<>(0.307, 0, -1.911965), ChVector<>(0.787, -0.118, -1.652965));
    my_system->AddLink(link_distRBL2);

    // .. create the spring between the truss and the spindle
    link_springRB = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springRB->Initialize(
        truss, spindleRB, false, ChVector<>(0.498, 0.323, -1.79297), ChVector<>(0.544, -0.038, -1.78597));
    ...
    link_distRBlat->Initialize(
        truss, spindleRB, false, ChVector<>(0.416, 0.059, -1.465965), ChVector<>(0.821, -0.009, -1.518965));
    ...
    link_distLBU1->Initialize(
        truss, spindleLB, false, ChVector<>(-0.462, 0.228, -1.339965), ChVector<>(-0.716, 0.216, -1.652965));
    my_system->AddLink(link_distLBU1);

    link_distLBU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, back, upper, 2
    link_distLBU2->Initialize(
        truss, spindleLB, false, ChVector<>(-0.462, 0.224, -1.611965), ChVector<>(-0.716, 0.216, -1.652965));
    my_system->AddLink(link_distLBU2);

    link_distLBL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, back, lower, 1
    link_distLBL1->Initialize(
        truss, spindleLB, false, ChVector<>(-0.307, 0, -1.465965), ChVector<>(-0.787, -0.118, -1.652965));
    my_system->AddLink(link_distLBL1);

    link_distLBL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance);  // left, back, lower, 2
    link_distLBL2->Initialize(
        truss, spindleLB, false, ChVector<>(-0.307, 0, -1.911965), ChVector<>(-0.787, -0.118, -1.652965));
    my_system->AddLink(link_distLBL2);

    // .. create the spring between the truss and the spindle
    link_springLB = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springLB->Initialize(
        truss, spindleLB, false, ChVector<>(-0.498, 0.323, -1.79297), ChVector<>(-0.544, -0.038, -1.78597));
    ...
    link_distLBlat->Initialize(
        truss, spindleLB, false, ChVector<>(-0.416, 0.059, -1.465965), ChVector<>(-0.821, -0.009, -1.518965));
...
void AddGround(ChSystemParallelDVI* sys) {
  ...
  utils::AddBoxGeometry(
      ground.get_ptr(), ChVector<>(groundWidth / 2, thickness, 10 * pitLocation_z / 2), ChVector<>(0, -thickness, 0));

  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, pitLocation_z/2), ChVector<>(0,
  // -thickness, pitLocation_z/2));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, pitLength/2), ChVector<>(0,
  // -thickness-pitDepth, pitLocation_z+pitLength/2));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness,
  // (groundLength-pitLength-pitLocation_z)/2), ChVector<>(0, -thickness,
  // pitLocation_z+pitLength+(groundLength-pitLength-pitLocation_z)/2));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, pitDepth/2, thickness), ChVector<>(0,
  // -pitDepth/2-thickness*2, pitLocation_z-thickness));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, pitDepth/2, thickness), ChVector<>(0,
  // -pitDepth/2-thickness*2, pitLocation_z+pitLength+thickness));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(thickness, wallHeight/2+pitDepth/2, groundLength/2),
  // ChVector<>(groundWidth/2+thickness, wallHeight/2-pitDepth/2-thickness*2, groundLength/2));
  // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(thickness, wallHeight/2+pitDepth/2, groundLength/2),
  // ChVector<>(-groundWidth/2-thickness, wallHeight/2-pitDepth/2-thickness*2, groundLength/2));
