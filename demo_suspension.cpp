...
public:
  ...
  // the ChBody object
  // .. truss:
  ChSharedBodyPtr truss;
  // .. right front suspension:
  ChSharedBodyPtr spindleRF;
  ChSharedBodyPtr wheelRF;
  ...
  ChSharedBodyPtr spindleLF;
  ChSharedBodyPtr wheelLF;
  ...
  ChSharedBodyPtr spindleRB;
  ChSharedBodyPtr wheelRB;
  ...
  ChSharedBodyPtr spindleLB;
  ChSharedBodyPtr wheelLB;
  ...
  MySimpleCar(ChSystemParallelDVI* my_system) {
    ...
    truss = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    utils::AddBoxGeometry(truss.get_ptr(), ChVector<>(0.7, 0.5, 3)*0.5, ChVector<>(0, 0, 0));
    ...
    spindleRF = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    utils::AddBoxGeometry(spindleRF.get_ptr(), ChVector<>(0.1, 0.4, 0.4)*0.5, ChVector<>(0, 0, 0));
    spindleRF->GetCollisionModel()->BuildModel();
    spindleRF->SetCollide(false);
    my_system->AddBody(spindleRF);

    // ..the car right-front wheel
    wheelRF = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    if(useSpheres)
    {
      utils::AddSphereGeometry(wheelRF.get_ptr(), 0.45, ChVector<>(0, 0, 0));
    }
    else
    {
      utils::AddCylinderGeometry(wheelRF.get_ptr(), 0.45, 0.2, ChVector<>(0, 0, 0), Q_from_AngZ(CH_C_PI_2));
    }
    ...
    link_revoluteRF->Initialize(wheelRF, spindleRF, ChCoordsys<>(ChVector<>(0.91, -0.026, 1.648965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_distRFU1->Initialize(truss, spindleRF, false, ChVector<>(0.446, 0.245, 1.640965), ChVector<>(0.716, 0.215, 1.635965));
    my_system->AddLink(link_distRFU1);

    link_distRFU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, front, upper, 2
    link_distRFU2->Initialize(truss, spindleRF, false, ChVector<>(0.478, 0.196, 1.420965), ChVector<>(0.716, 0.215, 1.635965));
    my_system->AddLink(link_distRFU2);

    link_distRFL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, front, lower, 1
    link_distRFL1->Initialize(truss, spindleRF, false, ChVector<>(0.307, 0, 1.911965), ChVector<>(0.787, -0.118, 1.652965));
    my_system->AddLink(link_distRFL1);

    link_distRFL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, front, lower, 2
    link_distRFL2->Initialize(truss, spindleRF, false, ChVector<>(0.307, 0, 1.465965), ChVector<>(0.787, -0.118, 1.652965));
    ...
    link_springRF->Initialize(truss, spindleRF, false, ChVector<>(0.498, 0.323, 1.792965), ChVector<>(0.543, -0.047, 1.785965));
    ...
    link_distRSTEER->Initialize(truss, spindleRF, false, ChVector<>(0.448, 0.054, 1.438965), ChVector<>(0.821, -0.016, 1.512965));
    my_system->AddLink(link_distRSTEER);

    // --- Left Front suspension ---

    // ..the car left-front spindle
    spindleLF = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    utils::AddBoxGeometry(spindleLF.get_ptr(), ChVector<>(0.1, 0.4, 0.4)*0.5, ChVector<>(0, 0, 0));
    spindleLF->GetCollisionModel()->BuildModel();
    spindleLF->SetCollide(false);
    my_system->AddBody(spindleLF);

    // ..the car left-front wheel
    wheelLF = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    if(useSpheres)
    {
      utils::AddSphereGeometry(wheelLF.get_ptr(), 0.45, ChVector<>(0, 0, 0));
    }
    else
    {
      utils::AddCylinderGeometry(wheelLF.get_ptr(), 0.45, 0.2, ChVector<>(0, 0, 0), Q_from_AngZ(CH_C_PI_2));
    }
    ...
    link_revoluteLF->Initialize(wheelLF, spindleLF, ChCoordsys<>(ChVector<>(-0.91, -0.026, 1.648965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    my_system->AddLink(link_revoluteLF);

    // .. impose distance between two parts (as a massless rod with two spherical joints at the end)
    link_distLFU1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); //left, front, upper, 1
    link_distLFU1->Initialize(truss, spindleLF, false, ChVector<>(-0.446, 0.245, 1.640965), ChVector<>(-0.716, 0.215, 1.635965));
    my_system->AddLink(link_distLFU1);

    link_distLFU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); //left, front, upper, 2
    link_distLFU2->Initialize(truss, spindleLF, false, ChVector<>(-0.478, 0.196, 1.420965), ChVector<>(-0.716, 0.215, 1.635965));
    my_system->AddLink(link_distLFU2);

    link_distLFL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, front, lower, 1
    link_distLFL1->Initialize(truss, spindleLF, false, ChVector<>(-0.307, 0, 1.911965), ChVector<>(-0.787, -0.118, 1.652965));
    my_system->AddLink(link_distLFL1);

    link_distLFL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, front, lower, 2
    link_distLFL2->Initialize(truss, spindleLF, false, ChVector<>(-0.307, 0, 1.465965), ChVector<>(-0.787, -0.118, 1.652965));
    my_system->AddLink(link_distLFL2);

    // .. create the spring between the truss and the spindle
    link_springLF = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springLF->Initialize(truss, spindleLF, false, ChVector<>(-0.498, 0.323, 1.792965), ChVector<>(-0.543, -0.047, 1.785965));
    ...
    link_distLSTEER->Initialize(truss, spindleLF, false, ChVector<>(-0.448, 0.054, 1.438965), ChVector<>(-0.821, -0.016, 1.512965));
    my_system->AddLink(link_distLSTEER);

    // --- Right Back suspension ---

    // ..the car right-back spindle
    spindleRB = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    utils::AddBoxGeometry(spindleRB.get_ptr(), ChVector<>(0.1, 0.4, 0.4)*0.5, ChVector<>(0, 0, 0));
    spindleRB->GetCollisionModel()->BuildModel();
    spindleRB->SetCollide(false);
    my_system->AddBody(spindleRB);

    // ..the car right-back wheel
    wheelRB = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    if(useSpheres)
    {
      utils::AddSphereGeometry(wheelRB.get_ptr(), 0.45, ChVector<>(0, 0, 0));
    }
    else
    {
      utils::AddCylinderGeometry(wheelRB.get_ptr(), 0.45, 0.2, ChVector<>(0, 0, 0), Q_from_AngZ(CH_C_PI_2));
    }
    ...
    link_revoluteRB->Initialize(wheelRB, spindleRB, ChCoordsys<>(ChVector<>(0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_engineR->Initialize(wheelRB, truss, ChCoordsys<>(ChVector<>(0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    ...
    link_engineR->Set_rot_funct(ChSharedPtr<ChFunction>(new ChFunction_Ramp(0, angularSpeed)));
    my_system->AddLink(link_engineR);

    // .. impose distance between two parts (as a massless rod with two spherical joints at the end)
    link_distRBU1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, back, upper, 1
    link_distRBU1->Initialize(truss, spindleRB, false, ChVector<>(0.462, 0.228, -1.339965), ChVector<>(0.716, 0.216, -1.652965));
    my_system->AddLink(link_distRBU1);

    link_distRBU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, back, upper, 2
    link_distRBU2->Initialize(truss, spindleRB, false, ChVector<>(0.462, 0.224, -1.611965), ChVector<>(0.716, 0.216, -1.652965));
    my_system->AddLink(link_distRBU2);

    link_distRBL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, back, lower, 1
    link_distRBL1->Initialize(truss, spindleRB, false, ChVector<>(0.307, 0, -1.465965), ChVector<>(0.787, -0.118, -1.652965));
    my_system->AddLink(link_distRBL1);

    link_distRBL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // right, back, lower, 2
    link_distRBL2->Initialize(truss, spindleRB, false, ChVector<>(0.307, 0, -1.911965), ChVector<>(0.787, -0.118, -1.652965));
    my_system->AddLink(link_distRBL2);

    // .. create the spring between the truss and the spindle
    link_springRB = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springRB->Initialize(truss, spindleRB, false, ChVector<>(0.498, 0.323, -1.79297), ChVector<>(0.544, -0.038, -1.78597));
    ...
    link_distRBlat->Initialize(truss, spindleRB, false, ChVector<>(0.416, 0.059, -1.465965), ChVector<>(0.821, -0.009, -1.518965));
    my_system->AddLink(link_distRBlat);

    // --- Left Back suspension ---

    // ..the car right-back spindle
    spindleLB = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    utils::AddBoxGeometry(spindleLB.get_ptr(), ChVector<>(0.1, 0.4, 0.4)*0.5, ChVector<>(0, 0, 0));
    spindleLB->GetCollisionModel()->BuildModel();
    spindleLB->SetCollide(false);
    my_system->AddBody(spindleLB);

    // ..the car left-back wheel
    wheelLB = (ChSharedPtr<ChBody>) (new ChBody(new ChCollisionModelParallel));
    ...
    if(useSpheres)
    {
      utils::AddSphereGeometry(wheelLB.get_ptr(), 0.45, ChVector<>(0, 0, 0));
    }
    else
    {
      utils::AddCylinderGeometry(wheelLB.get_ptr(), 0.45, 0.2, ChVector<>(0, 0, 0), Q_from_AngZ(CH_C_PI_2));
    }
    ...
    link_revoluteLB->Initialize(wheelLB, spindleLB, ChCoordsys<>(ChVector<>(-0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
    my_system->AddLink(link_revoluteLB);

    // .. create the motor transmission joint between the wheel and the truss (assuming small changes of alignment)
    link_engineL = ChSharedPtr<ChLinkEngine>(new ChLinkEngine);
    link_engineL->Initialize(wheelLB, truss, ChCoordsys<>(ChVector<>(-0.91, -0.026, -1.652965), chrono::Q_from_AngAxis(CH_C_PI / 2, VECT_Y)));
...
//    link_engineL->Set_eng_mode(ChLinkEngine::ENG_MODE_TORQUE);
    ...
    link_engineL->Set_rot_funct(ChSharedPtr<ChFunction>(new ChFunction_Ramp(0, angularSpeed)));
    my_system->AddLink(link_engineL);

    // .. impose distance between two parts (as a massless rod with two spherical joints at the end)
    link_distLBU1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, front, upper, 1
    link_distLBU1->Initialize(truss, spindleLB, false, ChVector<>(-0.462, 0.228, -1.339965), ChVector<>(-0.716, 0.216, -1.652965));
    my_system->AddLink(link_distLBU1);

    link_distLBU2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, back, upper, 2
    link_distLBU2->Initialize(truss, spindleLB, false, ChVector<>(-0.462, 0.224, -1.611965), ChVector<>(-0.716, 0.216, -1.652965));
    my_system->AddLink(link_distLBU2);

    link_distLBL1 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, back, lower, 1
    link_distLBL1->Initialize(truss, spindleLB, false, ChVector<>(-0.307, 0, -1.465965), ChVector<>(-0.787, -0.118, -1.652965));
    my_system->AddLink(link_distLBL1);

    link_distLBL2 = ChSharedPtr<ChLinkDistance>(new ChLinkDistance); // left, back, lower, 2
    link_distLBL2->Initialize(truss, spindleLB, false, ChVector<>(-0.307, 0, -1.911965), ChVector<>(-0.787, -0.118, -1.652965));
    my_system->AddLink(link_distLBL2);

    // .. create the spring between the truss and the spindle
    link_springLB = ChSharedPtr<ChLinkSpring>(new ChLinkSpring);
    link_springLB->Initialize(truss, spindleLB, false, ChVector<>(-0.498, 0.323, -1.79297), ChVector<>(-0.544, -0.038, -1.78597));
    ...
    link_distLBlat->Initialize(truss, spindleLB, false, ChVector<>(-0.416, 0.059, -1.465965), ChVector<>(-0.821, -0.009, -1.518965));
  ...
  double ComputeWheelTorque() {
    ...
    double shaftspeed = (1.0/this->conic_tau) * 0.5 * (this->link_engineL->Get_mot_rot_dt()+this->link_engineR->Get_mot_rot_dt());
    ...
    if (ChSharedPtr<ChFunction_Const> mfun = link_engineL->Get_tor_funct().DynamicCastTo<ChFunction_Const>()) mfun->Set_yconst(singlewheeltorque);
    if (ChSharedPtr<ChFunction_Const> mfun = link_engineR->Get_tor_funct().DynamicCastTo<ChFunction_Const>()) mfun->Set_yconst(singlewheeltorque);
...
int CreateGranularMaterial(ChSystemParallel* system, double pitLocation_z, double pitDepth, double pitLength, double groundWidth)
{
...
void AddGround(ChSystemParallelDVI* sys) {
  ...
  utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, 10*pitLocation_z/2), ChVector<>(0, -thickness, 0));

  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, pitLocation_z/2), ChVector<>(0, -thickness, pitLocation_z/2));
  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, pitLength/2), ChVector<>(0, -thickness-pitDepth, pitLocation_z+pitLength/2));
  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, thickness, (groundLength-pitLength-pitLocation_z)/2), ChVector<>(0, -thickness, pitLocation_z+pitLength+(groundLength-pitLength-pitLocation_z)/2));
  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, pitDepth/2, thickness), ChVector<>(0, -pitDepth/2-thickness*2, pitLocation_z-thickness));
 // utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(groundWidth/2, pitDepth/2, thickness), ChVector<>(0, -pitDepth/2-thickness*2, pitLocation_z+pitLength+thickness));
  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(thickness, wallHeight/2+pitDepth/2, groundLength/2), ChVector<>(groundWidth/2+thickness, wallHeight/2-pitDepth/2-thickness*2, groundLength/2));
  //utils::AddBoxGeometry(ground.get_ptr(), ChVector<>(thickness, wallHeight/2+pitDepth/2, groundLength/2), ChVector<>(-groundWidth/2-thickness, wallHeight/2-pitDepth/2-thickness*2, groundLength/2));
